...
Error JITLinkExecutor::addDebugSupport(ObjectLayer &ObjLayer) {
  ...
  if (ObjLinkingLayer == nullptr)
    return make_error<StringError>("", inconvertibleErrorCode());

  auto Registrar = ::llvm::orc::createJITLoaderGDBRegistrar(*TPC);
  if (!Registrar)
    return Registrar.takeError();

  ObjLinkingLayer->addPlugin(std::make_unique<DebugObjectManagerPlugin>(
      ObjLinkingLayer->getExecutionSession(), std::move(*Registrar)));
...
Expected<std::unique_ptr<ChildProcessJITLinkExecutor>>
JITLinkExecutor::FindLocal(const char *JITArgv0) {
  std::string ExecutablePath = ChildProcessJITLinkExecutor::defaultPath(
      JITArgv0, "llvm-jitlink-executor");
  if (!sys::fs::can_execute(ExecutablePath))
...
std::string ChildProcessJITLinkExecutor::defaultPath(const char *JITArgv0,
                                                     StringRef ExecutorName) {
  SmallString<256> FullName(sys::fs::getMainExecutable(
      JITArgv0, reinterpret_cast<void *>(&unusedLocalFunction)));
...
static Expected<int> connectTCPSocketImpl(std::string Host, std::string PortStr) {
  addrinfo *AI;
  addrinfo Hints{};
  ...
  if (Server == nullptr)
    return make_error<StringError>("invalid hostname", inconvertibleErrorCode());
