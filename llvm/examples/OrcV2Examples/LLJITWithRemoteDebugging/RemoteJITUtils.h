...
public:
  ...
  /// Create a JITLinkExecutor that connect to the given network address through
  /// a TCP socket. Valid NetworkAddresses provide hostname and port, e.g.
  /// localhost:20000.
  static Expected<std::unique_ptr<TCPSocketJITLinkExecutor>>
  ConnectTCPSocket(StringRef NetworkAddress, ExecutionSession &ES);

  // Implements ObjectLinkingLayerCreator
