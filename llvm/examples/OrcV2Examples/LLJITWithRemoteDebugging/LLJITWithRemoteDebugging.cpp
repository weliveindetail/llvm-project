...
//===----------------------------------------------------------------------===//
//
// This example shows how to use LLJIT with JITLink for out-of-process execution
// with debug support.  A few notes beforehand:
//
//  * Debug support is currently limited to ELF on x86-64 platforms that run
//    Unix-like systems.
//  * The test for this example brings a IR code file that is suitable for
//    trying the feature.
...
static cl::opt<std::string> InputFile(cl::desc("<input IR code>"),
                                      cl::Positional, cl::ValueRequired);

// Command line arguments to pass to the JITed main function.
static cl::list<std::string> InputArgv(
    cl::ConsumeAfter, cl::desc("<program arguments>..."));
...
static cl::opt<std::string>
    OutOfProcessExecutor("oop-executor",
                         cl::desc("Set the out-of-process executor"),
...
static std::unique_ptr<JITLinkExecutor> connectExecutor(const char *Argv0,
  ...
  if (OutOfProcessExecutorConnect.getNumOccurrences() > 0) {
    std::unique_ptr<TCPSocketJITLinkExecutor> Exec = ExitOnErr(
        JITLinkExecutor::ConnectTCPSocket(OutOfProcessExecutorConnect, ES));

    outs() << "Connected to executor at " << OutOfProcessExecutorConnect << "\n";
    if (WaitForDebugger) {
  ...
  std::unique_ptr<ChildProcessJITLinkExecutor> Exec =
      ExitOnErr(OutOfProcessExecutor.empty()
                    ? JITLinkExecutor::FindLocal(Argv0)
                    : JITLinkExecutor::CreateLocal(OutOfProcessExecutor));
...
int main(int argc, char *argv[]) {
  ...
  outs() << "Parsed input IR code from: " << InputFile << "\n";

  // Create a target machine that matches the triple of the given module.
  const std::string &TT = TSM.getModuleUnlocked()->getTargetTriple();
  ...
  // Create LLJIT and destroy it before disconnecting the target process.
  {
    ...
    outs() << "Initialized LLJIT for remote executor\n";

    // Add the loaded IR module to the JIT. This will process symbol information
    // and prepare for materialization.
    ExitOnErr(J->addIRModule(std::move(TSM)));

    // Looking up the main function materializes the code. This also triggers
    // debug registration in the remote target process.
    JITEvaluatedSymbol MainFn = ExitOnErr(J->lookup("main"));

    outs() << "Running: " << InputFile;
