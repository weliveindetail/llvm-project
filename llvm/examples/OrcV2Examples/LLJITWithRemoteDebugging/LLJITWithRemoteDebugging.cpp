...
static std::unique_ptr<shared::FDRawByteChannel> connectRPC(const char *Argv0) {
  std::unique_ptr<shared::FDRawByteChannel> Channel;

  if (OOPExecutorConnect.empty()) {
    // Launch a out-of-process executor locally in a child process.
    std::string Path = OOPExecutor.empty() ? findLocalExecutor(Argv0) : OOPExecutor;
    outs() << "Found out-of-process executor: " << Path << "\n";

    pid_t PID;
    std::tie(Channel, PID) = ExitOnErr(launchLocalExecutor(Path));
    outs() << "Launched executor in subprocess: " << PID << "\n";
  } else {
    // Connect to a running out-of-process executor through a TCP socket.
    Channel = ExitOnErr(connectTCPSocket(OOPExecutorConnect));
...
int main(int argc, char *argv[]) {
  ...
  auto Channel = connectRPC(argv[0]);
  ...
  auto &TPC = J->getExecutionSession().getTargetProcessControl();
  int Result = ExitOnErr(TPC.runAsMain(MainFn.getAddress(), InputArgv));
