...
void DebugObjectManagerPlugin::notifyMaterializing(
  ...
  if (auto DebugObj = createDebugObjectFromBuffer(G, Ctx, ObjBuffer)) {
    ...
    if (*DebugObj)
      PendingObjs[Key].push_back(std::move(*DebugObj));
  } else {
...
void DebugObjectManagerPlugin::modifyPassConfig(
  ...
  for (std::unique_ptr<DebugObject> &Entry : It->second) {
    DebugObject &DebugObj = *Entry;
      ...
      PassConfig.PostAllocationPasses.push_back(
          [&DebugObj](LinkGraph &Graph) -> Error {
            for (const Section &GraphSection : Graph.sections())
              DebugObj.reportSectionTargetMemoryRange(
                  GraphSection.getName(), SectionRange(GraphSection));
            return Error::success();
          });
    }
  }
...
Error DebugObjectManagerPlugin::notifyEmitted(
  ...
  DebugObjects DebugObjs;
  {
    ...
    DebugObjs = std::move(It->second);
    PendingObjs.erase(It);
  }

  for (std::unique_ptr<DebugObject> &Entry : DebugObjs) {
    // FIXME: We release ownership of DebugObj, so we can easily capture the raw
    // pointer in the continuation function, which re-owns it immediately.
    DebugObject *UnownedDebugObj = Entry.release();
...
void DebugObjectManagerPlugin::notifyTransferringResources(ResourceKey DstKey,
  ...
  }
  {
    ...
    if (SrcIt != PendingObjs.end()) {
      for (std::unique_ptr<DebugObject> &DebugObj : SrcIt->second)
        PendingObjs[DstKey].push_back(std::move(DebugObj));
