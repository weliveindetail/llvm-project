...
const char *getEdgeKindName(Edge::Kind Kind) {
  ...
  // we must return a raw c-string here. We know that getELFRelocationTypeName()
  // returns a string literal, so it might be ok for now. It's not a good
  // pattern for the future though.
  return Name.data();
}
...
private:
  ...
  Error applyFixup(LinkGraph &G, Block &B, const Edge &E) const {
    assert(!errorToBool(getELFRelocationType(E.getKind()).takeError()) &&
           "Can not fix-up relocation type: AArch32 edges only");
...
private:
  ...
  Error addSingleRelRelocation(const typename ELFT::Rel &Rel,
    ...
    LLVM_DEBUG({
      dbgs() << "    ";
      printEdge(dbgs(), BlockToFix, E, getEdgeKindName(*Kind));
      dbgs() << "\n";
    });
