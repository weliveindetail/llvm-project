...
Expected<Session::MemoryRegionInfo &>
Session::findStubInfo(StringRef FileName, StringRef TargetName, uint64_t StubIndex) {
  auto FI = findFileInfo(FileName);
  if (!FI)
    return FI.takeError();
  ...
  if (StubIndex >= StubInfoItr->second.size())
    return make_error<StringError>("no stub for \"" + TargetName + " with index " + Twine(StubIndex) +
                                       "\" registered for file \"" + FileName +
                                       "\"",
                                   inconvertibleErrorCode());
...
static Error runChecks(Session &S, Triple TT, SubtargetFeatures Features) {
  ...
  auto GetStubInfo = [&S](StringRef FileName, StringRef SectionName, int64_t StubIndex) {
