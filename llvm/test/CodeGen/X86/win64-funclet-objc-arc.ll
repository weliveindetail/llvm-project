; RUN: opt -mtriple=x86_64-windows-msvc -objc-arc -S -o - < %s | FileCheck %s

; Check that the objc-arc pass preserves funclet tokens
;
; CHECK-LABEL:  catch:
; CHECK:          %1 = catchpad within %0
; CHECK:          %2 = tail call ptr @llvm.objc.retain(ptr %exn) #0
; CHECK-NOT:      [ "funclet"(token %1) ]
; CHECK:          %3 = tail call ptr @llvm.objc.retain(ptr %exn) #0
; CHECK:          [ "funclet"(token %1) ]
; CHECK:          call void @llvm.objc.release(ptr %exn) #0
; CHECK:          [ "funclet"(token %1) ]
...
eh.cont:                                          ; preds = %catch, %entry
  ...
  %2 = call ptr @llvm.objc.retain(ptr %exn)
  %3 = call ptr @llvm.objc.retain(ptr %exn) [ "funclet"(token %1) ]
